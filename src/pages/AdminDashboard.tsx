...
import { Save, Loader2, Users, Database, Trash2, CheckCircle, XCircle, Search, KeyRound, X, Printer, FileText, Truck, ShieldCheck, UserPlus } from 'lucide-react';
...
export default function AdminDashboard() {
    ...
    useEffect(() => {
        ...
        if (tab === 'users' && filter) {
            const validFilters = ['PENDING', Role.STAGING_SUPERVISOR, Role.LOADING_SUPERVISOR, Role.SHIFT_LEAD];
    ...
    const handleTabChange = (tab: string) => {
        setActiveTab(tab);
        setSubFilter('ALL');
        // Reset user filter when leaving users tab
    ...
    const handleDeleteUser = async (userToDelete: any) => {
        // debug: confirm click is registered
        // alert(`Debug: Clicked delete for ${userToDelete.username}`);
    ...
    const UserManagement = () => (
        <div className="space-y-6 animate-in fade-in zoom-in-95 duration-300">
            <div className="flex items-center justify-between">
                ...
                <div className="flex bg-muted p-1 rounded-lg gap-1 overflow-x-auto">
                    <button
                        ...
                        className={`text-xs px-3 py-1.5 rounded-md font-medium transition-all whitespace-nowrap ${userFilter === 'ALL' ? 'bg-background text-foreground shadow-sm' : 'text-muted-foreground hover:text-foreground'}`}
                    >
                        All Users
                    </button>
                    <button
                        onClick={() => setUserFilter('PENDING')}
                        className={`text-xs px-3 py-1.5 rounded-md font-medium transition-all flex items-center gap-2 whitespace-nowrap ${userFilter === 'PENDING' ? 'bg-background text-foreground shadow-sm' : 'text-muted-foreground hover:text-foreground'}`}
                    >
                        Pending
                        {pendingUsersCount > 0 && <span className="bg-destructive text-destructive-foreground text-[10px] px-1.5 py-0.5 rounded-full min-w-[1.25rem] text-center">{pendingUsersCount}</span>}
                    </button>
                    <div className="w-px h-6 bg-border mx-1 my-auto hidden md:block"></div>
                    <button
                        onClick={() => setUserFilter(Role.STAGING_SUPERVISOR)}
                        className={`text-xs px-3 py-1.5 rounded-md font-medium transition-all whitespace-nowrap ${userFilter === Role.STAGING_SUPERVISOR ? 'bg-blue-500/10 text-blue-500 shadow-sm border border-blue-500/20' : 'text-muted-foreground hover:text-blue-500'}`}
                    >
                        Staging
                    </button>
                    <button
                        onClick={() => setUserFilter(Role.LOADING_SUPERVISOR)}
                        className={`text-xs px-3 py-1.5 rounded-md font-medium transition-all whitespace-nowrap ${userFilter === Role.LOADING_SUPERVISOR ? 'bg-orange-500/10 text-orange-500 shadow-sm border border-orange-500/20' : 'text-muted-foreground hover:text-orange-500'}`}
                    >
                        Loading
                    </button>
                    <button
                        onClick={() => setUserFilter(Role.SHIFT_LEAD)}
                        className={`text-xs px-3 py-1.5 rounded-md font-medium transition-all whitespace-nowrap ${userFilter === Role.SHIFT_LEAD ? 'bg-purple-500/10 text-purple-500 shadow-sm border border-purple-500/20' : 'text-muted-foreground hover:text-purple-500'}`}
                    >
                        Shift Lead
                    </button>
                </div>
                ...
                <div className="flex items-center gap-2">
                    <div className="relative">
                        ...
                        <input
                            ...
                            className="bg-background border border-input rounded-lg pl-9 pr-3 py-2 text-sm text-foreground placeholder-muted-foreground focus:border-primary focus:outline-none w-48"
                            placeholder="Search users..."
                        />
                    </div>
                    <Button onClick={() => setIsAddingUser(!isAddingUser)} size="sm" className="bg-primary hover:bg-primary/90 text-primary-foreground">
                    ...
                    <form onSubmit={handleAddUser} className="space-y-4">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="space-y-1">
                                ...
                                <input required value={newUser.username} onChange={e => setNewUser({ ...newUser, username: e.target.value })} className="w-full bg-background border border-input rounded px-3 py-2 text-foreground placeholder-muted-foreground focus:border-primary outline-none font-bold" placeholder="e.g. johndoe" />
                            </div>
                            <div className="space-y-1">
                                <label className="text-xs text-muted-foreground">Full Name</label>
                                <input value={newUser.fullName} onChange={e => setNewUser({ ...newUser, fullName: e.target.value })} className="w-full bg-background border border-input rounded px-3 py-2 text-foreground placeholder-muted-foreground focus:border-primary outline-none font-bold" placeholder="e.g. John Doe" />
                            </div>
                            <div className="space-y-1">
                                <label className="text-xs text-muted-foreground">Email Address</label>
                                <input type="email" value={newUser.email} onChange={e => setNewUser({ ...newUser, email: e.target.value })} className="w-full bg-background border border-input rounded px-3 py-2 text-foreground placeholder-muted-foreground focus:border-primary outline-none font-bold" placeholder="john@example.com" />
                            </div>
                            <div className="space-y-1">
                                <label className="text-xs text-muted-foreground">EMP Code</label>
                                <input value={newUser.empCode} onChange={e => setNewUser({ ...newUser, empCode: e.target.value })} className="w-full bg-background border border-input rounded px-3 py-2 text-foreground placeholder-muted-foreground focus:border-primary outline-none font-bold" placeholder="e.g. EMP123" />
                            </div>
                            <div className="space-y-1">
                                <label className="text-xs text-muted-foreground">Initial Password <span className="text-destructive">*</span></label>
                                <input type="password" required value={newUser.password} onChange={e => setNewUser({ ...newUser, password: e.target.value })} className="w-full bg-background border border-input rounded px-3 py-2 text-foreground placeholder-muted-foreground focus:border-primary outline-none font-bold" placeholder="******" />
                            </div>
                            <div className="space-y-1 md:col-span-2">
                                <label className="text-xs text-muted-foreground">Role</label>
                                <select value={newUser.role} onChange={e => setNewUser({ ...newUser, role: e.target.value as Role })} className="w-full bg-background border border-input rounded px-3 py-2 text-foreground focus:border-primary outline-none font-bold">
                        ...
                        {filteredUsers.length === 0 ? (
                            ...
                            <tr key={user.id} className="group hover:bg-slate-50 dark:hover:bg-white/5 transition-colors">
                                <td className="py-3 pl-4">
                                    <div className="font-medium text-slate-900 dark:text-slate-200">{user.fullName || user.username}</div>
                                    <div className="text-xs text-slate-500 dark:text-slate-500">@{user.username}</div>
                                </td>
                                <td className="py-3"><Badge variant="outline" className="text-xs border-slate-200 dark:border-white/10 bg-slate-100 dark:bg-slate-950 text-slate-600 dark:text-slate-400">{user.role}</Badge></td>
                                <td className="py-3 text-xs text-slate-600 dark:text-slate-400 font-mono">{user.empCode || '-'}</td>
                                <td className="py-3">
                                ...
                                <td className="py-3 pr-4 text-right flex justify-end gap-2">
                                    ...
                                    <Button variant="ghost" size="sm" className="h-8 w-8 p-0 text-blue-500 hover:text-blue-400 hover:bg-blue-500/10" onClick={() => setEditingUser(user)} title="Edit User">
                                        <FileText size={14} />
                                    </Button>
                                    {user.id !== currentUser?.id && (
                                        <Button
                                            variant="ghost"
                                            size="sm"
                                            className={`h-8 w-8 p-0 ${user.isApproved ? 'text-slate-600' : 'text-red-500 hover:text-red-400 hover:bg-red-500/10'}`}
            ...
            <AnimatePresence>
                {editingUser && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
                        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="w-full max-w-lg bg-slate-900 border border-white/10 rounded-2xl shadow-2xl overflow-hidden">
                            <div className="p-6 border-b border-white/10 flex justify-between items-center bg-slate-950/50">
                                <div>
                                    <h3 className="text-lg font-bold text-white">Edit User</h3>
                                    <p className="text-sm text-slate-400">Update details for {editingUser.username}</p>
                                </div>
                                <button onClick={() => setEditingUser(null)} className="text-slate-400 hover:text-white transition-colors"><X size={20} /></button>
                            </div>
                            <form onSubmit={handleUpdateUser} className="p-6 space-y-4">
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="space-y-1">
                                        ...
                                        <input
                                            ...
                                            className="w-full bg-black/40 border border-white/10 rounded-lg px-3 py-2 text-white focus:outline-none focus:border-blue-500 text-sm"
                                        />
                                    </div>
                                    <div className="space-y-1">
                                        <label className="text-xs font-medium text-slate-400">EMP Code</label>
                                        <input
                                ...
                                <div className="pt-4 flex justify-end gap-3 border-t border-white/5 mt-4">
                                    <Button type="button" variant="ghost" onClick={() => setEditingUser(null)} className="text-slate-400 hover:text-white">Cancel</Button>
                                    <Button type="submit" className="bg-blue-600 hover:bg-blue-500 text-white">Save Changes</Button>
                                </div>
                            </form>
                        </motion.div>
                    </div>
                    ...
                    {currentUser && (
                        <div className="flex items-center gap-2">
                            <div className="text-right">
                                <div className="text-sm font-bold text-slate-900 dark:text-white">{currentUser.username}</div>
