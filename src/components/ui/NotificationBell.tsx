...
export function NotificationBell() {
    const { sheets, currentUser, settings } = useData();
    ...
    const notifications = useMemo(() => {
        ...
        return sheets.filter(sheet => {
            ...
            if (currentUser.role === Role.SHIFT_LEAD || currentUser.role === Role.ADMIN) {
                return sheet.status === SheetStatus.STAGING_VERIFICATION_PENDING ||
    ...
    if (notifications.length === 0) {
        return (
            <Button variant="ghost" size="icon" className="text-muted-foreground opacity-50 relative">
                <Bell size={20} />
            </Button>
        );
    }
            ...
            <PopoverContent className="w-80 p-0 mr-4" align="end">
                <div className="p-4 border-b border-border bg-muted/30">
                    <div className="flex items-center justify-between">
                        ...
                        <Badge variant="secondary" className="bg-red-100 text-red-700 dark:bg-red-500/20 dark:text-red-300">
                        ...
                        {notifications.map(sheet => (
                            ...
                            >
                                <div className={cn(
                                    ...
                                    sheet.rejectionReason ? "bg-red-100 text-red-600" : "bg-blue-100 text-blue-600"
                                )}>
                                    {sheet.rejectionReason ? <AlertTriangle size={14} /> : <CheckCircle size={14} />}
                                </div>
                                <div className="flex-1 min-w-0">
                                    <div className="flex justify-between items-start">
                                        <span className="font-medium text-sm truncate">Sheet #{sheet.id}</span>
                ...
                <div className="p-2 border-t border-border bg-muted/30 text-center">
                    <Button variant="ghost" size="sm" className="w-full h-8 text-xs text-muted-foreground" onClick={() => setOpen(false)}>
