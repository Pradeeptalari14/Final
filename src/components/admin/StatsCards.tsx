        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 animate-in fade-in slide-in-from-top-4">
            {(currentUser?.role === Role.ADMIN ||
                ...
                >
                    <CardContent className="p-6 relative z-10 flex flex-col justify-between h-full">
                        ...
                        >
                            <div
                                className={`p-3 rounded-xl backdrop-blur-sm ${activeSection === 'staging_db' ? 'bg-white/20 text-white' : 'bg-blue-50 text-blue-600'}`}
                            >
                                <FileText size={24} />
                            </div>
                            <div
                                className={`text-2xl font-bold ${activeSection === 'staging_db' ? 'text-white' : 'text-slate-800'}`}
                            >
                                {stats.staging}
                            </div>
                        </div>
                        ...
                        <div className="flex justify-between items-end">
                            ...
                            >
                                <div
                                    className={`font-semibold mb-1 ${activeSection === 'staging_db' ? 'text-blue-50' : 'text-slate-600'}`}
                                >
                                    {t('staging_tasks', settings.language)}
                                </div>
                                <div
                                    className={`text-xs font-medium ${activeSection === 'staging_db' ? 'text-blue-100' : 'text-slate-400'}`}
                                >
                                    {stats.staging > 0
            ...
            {(currentUser?.role === Role.ADMIN ||
                ...
                >
                    <CardContent className="p-6 relative z-10 flex flex-col justify-between h-full">
                        ...
                        >
                            <div
                                className={`p-3 rounded-xl backdrop-blur-sm ${activeSection === 'loading_db' ? 'bg-white/20 text-white' : 'bg-orange-50 text-orange-600'}`}
                            >
                                <Truck size={24} />
                            </div>
                            <div
                                className={`text-2xl font-bold ${activeSection === 'loading_db' ? 'text-white' : 'text-slate-800'}`}
                            >
                                {stats.loading}
                            </div>
                        </div>
                        ...
                        <div className="flex justify-between items-end">
                            ...
                            >
                                <div
                                    className={`font-semibold mb-1 ${activeSection === 'loading_db' ? 'text-orange-50' : 'text-slate-600'}`}
                                >
                                    {t('loading_tasks', settings.language)}
                                </div>
                                <div
                                    className={`text-xs font-medium ${activeSection === 'loading_db' ? 'text-orange-100' : 'text-slate-400'}`}
                                >
                                    {stats.loading > 0
                                        ? t('active', settings.language)
                                        : t('no_active_tasks', settings.language)}
                    ...
                    <CardContent className="p-6 relative z-10 p-0">
                        ...
                        >
                            <div className="flex justify-between items-start mb-4">
                                <div
                                    className={`p-3 rounded-xl backdrop-blur-sm ${activeSection === 'shift_lead_db' ? 'bg-white/20 text-white' : 'bg-purple-50 text-purple-600'}`}
                                >
                                    <ShieldCheck size={24} />
                                </div>
                                <div
                                    className={`text-2xl font-bold ${activeSection === 'shift_lead_db' ? 'text-white' : 'text-slate-800'}`}
                                >
                                    {stats.approval}
                                </div>
                            </div>
                            <div className="flex justify-between items-end">
                                <div>
                                    <div
                                        className={`font-semibold mb-1 ${activeSection === 'shift_lead_db' ? 'text-purple-50' : 'text-slate-600'}`}
                                    >
                                        {t('shift_lead', settings.language)}
                                    </div>
                                    <div
                                        className={`text-xs font-medium ${activeSection === 'shift_lead_db' ? 'text-purple-100' : 'text-slate-400'}`}
                                    >
                                        {stats.approval > 0
            ...
            >
                <CardContent className="p-6 relative z-10">
                    <div className="flex justify-between items-start mb-4">
                        <div
                            className={`p-3 rounded-xl backdrop-blur-sm ${activeSection === 'database' ? 'bg-white/20 text-white' : 'bg-indigo-50 text-indigo-600'}`}
                        >
                            <Database size={24} />
                        </div>
                        <div
                            className={`text-2xl font-bold ${activeSection === 'database' ? 'text-white' : 'text-slate-800'}`}
                        >
                            •
                        </div>
                    </div>
                    <div>
                        <div
                            className={`font-semibold mb-1 ${activeSection === 'database' ? 'text-indigo-50' : 'text-slate-600'}`}
                        >
                            {t('master_records', settings.language)}
                        </div>
                        <div
                            className={`text-xs font-medium ${activeSection === 'database' ? 'text-indigo-100' : 'text-slate-400'}`}
                        >
