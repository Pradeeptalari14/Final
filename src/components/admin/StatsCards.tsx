...
import { Truck, ShieldCheck, Database, FileText, LayoutDashboard as GripVertical } from 'lucide-react';
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 animate-in fade-in slide-in-from-top-4">
                ...
                >
                    <CardContent className="p-4 flex items-center gap-3">
                        ...
                        <div className="flex-1">
                            <div className="font-bold text-slate-800 dark:text-slate-100">{t('staging_tasks', settings.language)}</div>
                            <div className="text-xs font-semibold text-slate-500 dark:text-slate-400">
                                {stats.staging > 0 ? <span className="text-blue-600">{stats.staging} {t('active', settings.language)}</span> : t('no_active_tasks', settings.language)}
                            </div>
                        </div>
                    </CardContent>
                </Card>
            ...
            {(currentUser?.role === Role.ADMIN || currentUser?.role === Role.LOADING_SUPERVISOR) && (
                ...
                >
                    <CardContent className="p-4 flex items-center gap-3">
                        ...
                        <div className="flex-1">
                            <div className="font-bold text-slate-800 dark:text-slate-100">{t('loading_tasks', settings.language)}</div>
                            <div className="text-xs font-semibold text-slate-500 dark:text-slate-400">
                                {stats.loading > 0 ? <span className="text-orange-600">{stats.loading} {t('active', settings.language)}</span> : t('no_active_tasks', settings.language)}
                            </div>
                        </div>
                    </CardContent>
                </Card>
            ...
            {(currentUser?.role === Role.ADMIN || currentUser?.role === Role.SHIFT_LEAD) && (
                ...
                >
                    <CardContent className="p-4 flex items-center gap-3">
                        ...
                        <div className="flex-1">
                            <div className="font-bold text-slate-800 dark:text-slate-100">{t('shift_lead', settings.language)}</div>
                            <div className="text-xs font-semibold text-slate-500 dark:text-slate-400">
                                {stats.approval > 0 ? <span className="text-purple-600">{stats.approval} {t('needs_approval', settings.language)}</span> : t('all_approved', settings.language)}
                            </div>
                        </div>
                    </CardContent>
                </Card>
            ...
            >
                <CardContent className="p-4 flex items-center gap-3">
                    ...
                    <div>
                        <div className="font-bold text-slate-800 dark:text-slate-100">{t('master_records', settings.language)}</div>
                        <div className="text-xs font-semibold text-slate-500 dark:text-slate-400">{t('history_title', settings.language)}</div>
                    </div>
                </CardContent>
            </Card>
        </div>
