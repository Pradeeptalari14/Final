...
export function StatsCards({ currentUser, activeSection, onNavigate, stats, density = 'comfortable' }: StatsCardsProps) {
    const { settings } = useAppState();
    const isCompact = density === 'compact';

    return (
        <div className={`grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 ${isCompact ? 'gap-2' : 'gap-6'} animate-in fade-in slide-in-from-top-4`}>
            {(currentUser?.role === Role.ADMIN ||
                currentUser?.role === Role.STAGING_SUPERVISOR) && (
                    ...
                    >
                        <CardContent className={`${isCompact ? 'p-2.5' : 'p-6'} relative z-10 flex flex-col justify-between h-full`}>
                            {/* CLICKABLE HEADER ZONE - Explicit View Mode */}
                            <div
                                className={`flex justify-between items-start ${isCompact ? 'mb-2' : 'mb-4'} cursor-pointer`}
                            ...
                            >
                                ...
                                <div
                                    className={`${isCompact ? 'text-lg' : 'text-2xl'} font-bold ${activeSection === 'staging_db' ? 'text-white' : 'text-slate-800'}`}
                                >
                                    {stats.staging}
                                </div>
                            </div>
                            ...
                            <div className="flex justify-between items-end">
                                ...
                                >
                                    <div
                                        className={`${isCompact ? 'text-xs' : 'font-semibold'} mb-0.5 ${activeSection === 'staging_db' ? 'text-blue-50' : 'text-slate-600'}`}
                                    >
                                        {t('staging_tasks', settings.language)}
                                    </div>
                                    <div
                                        className={`text-[10px] sm:text-xs font-medium ${activeSection === 'staging_db' ? 'text-blue-100' : 'text-slate-400'}`}
                                    >
                                        {stats.staging > 0
                                            ? t('active', settings.language)
                                            : t('no_active_tasks', settings.language)}
                                    </div>
                                </div>
                                ...
                                <div className="flex flex-wrap gap-1 z-50 relative">
                                    {stats.stagingDrafts !== undefined && stats.stagingDrafts > 0 && (
                                        <button
                                            ...
                                            onClick={(e) => {
                                                e.stopPropagation();
                                                onNavigate('staging_db', 'VIEW_STAGING_DRAFT');
                                            }}
                                    ...
                                    {stats.stagingPending !== undefined && stats.stagingPending > 0 && (
                                        <button
                                            ...
                                            onClick={(e) => {
                                                e.stopPropagation();
                                                onNavigate('staging_db', 'VIEW_STAGING_VERIFY');
                                            }}
                                            className={`relative z-50 hover:scale-105 active:scale-95 px-2 py-1 rounded text-[10px] font-bold transition-all ${activeSection === 'staging_db' ? 'bg-white/20 hover:bg-white/30 text-white' : 'bg-blue-50 text-blue-600 hover:bg-blue-100'}`}
                                        >
                    ...
                    >
                        <CardContent className={`${isCompact ? 'p-2.5' : 'p-6'} relative z-10 flex flex-col justify-between h-full`}>
                            ...
                            >
                                ...
                                <div
                                    className={`${isCompact ? 'text-lg' : 'text-2xl'} font-bold ${activeSection === 'loading_db' ? 'text-white' : 'text-slate-800'}`}
                                >
                                    {stats.loading}
                                </div>
                            </div>
                            ...
                            <div className="flex justify-between items-end">
                                ...
                                >
                                    <div
                                        className={`${isCompact ? 'text-xs' : 'font-semibold'} mb-0.5 ${activeSection === 'loading_db' ? 'text-orange-50' : 'text-slate-600'}`}
                                    >
                                        {t('loading_tasks', settings.language)}
                                    </div>
                                    <div
                                        className={`text-[10px] sm:text-xs font-medium ${activeSection === 'loading_db' ? 'text-orange-100' : 'text-slate-400'}`}
                                    >
                                        {stats.loading > 0
                                            ? t('active', settings.language)
                                            : t('no_active_tasks', settings.language)}
                                ...
                                <div className="flex flex-wrap gap-1 z-50 relative">
                                    {stats.loadingReady !== undefined && stats.loadingReady > 0 && (
                                        <button
                                            ...
                                            onClick={(e) => {
                                                e.stopPropagation();
                                                onNavigate('loading_db', 'VIEW_LOADING_READY');
                                            }}
                                    ...
                                    {stats.loadingPending !== undefined && stats.loadingPending > 0 && (
                                        <button
                                            ...
                                            onClick={(e) => {
                                                e.stopPropagation();
                                                onNavigate('loading_db', 'VIEW_LOADING_VERIFY');
                                            }}
                                            className={`relative z-50 hover:scale-105 active:scale-95 px-2 py-1 rounded text-[10px] font-bold transition-all ${activeSection === 'loading_db' ? 'bg-white/20 hover:bg-white/30 text-white' : 'bg-orange-50 text-orange-600 hover:bg-orange-100'}`}
                                        >
                                            {stats.loadingPending}{' '}
                                            {t('pending_verification', settings.language)}
                                        </button>
                                    )}
                                </div>
            ...
            {(currentUser?.role === Role.ADMIN || currentUser?.role === Role.SHIFT_LEAD) && (
                ...
                >
                    <CardContent className={`${isCompact ? 'p-0' : 'p-6 relative z-10 p-0'}`}>
                        {/* WRAPPER DIV for Shift Lead Click Zone - Default to Staging Verify */}
                        <div
                            className={`${isCompact ? 'p-4' : 'p-6'} h-full flex flex-col justify-between cursor-pointer`}
                            onClick={() => onNavigate('shift_lead_db', 'VIEW_STAGING_VERIFY')}
                        >
                            <div className={`flex justify-between items-start ${isCompact ? 'mb-1' : 'mb-4'}`}>
                                ...
                                <div
                                    className={`${isCompact ? 'text-lg' : 'text-2xl'} font-bold ${activeSection === 'shift_lead_db' ? 'text-white' : 'text-slate-800'}`}
                                >
                                    {stats.approval}
                                </div>
                            </div>
                            <div className="flex justify-between items-end">
                                <div>
                                    <div
                                        className={`${isCompact ? 'text-xs' : 'font-semibold'} mb-0.5 ${activeSection === 'shift_lead_db' ? 'text-purple-50' : 'text-slate-600'}`}
                                    >
                                        {t('shift_lead', settings.language)}
                                    </div>
                                    <div
                                        className={`text-[10px] sm:text-xs font-medium ${activeSection === 'shift_lead_db' ? 'text-purple-100' : 'text-slate-400'}`}
                                    >
                                        {stats.approval > 0
                                            ? t('needs_approval', settings.language)
                                            : t('all_approved', settings.language)}
                                    </div>
                                </div>
                                <div className="flex gap-1">
                                    {stats.stagingPending !== undefined &&
                                        stats.stagingPending > 0 && (
                                            <button
                                                type="button"
                                                onClick={(e) => {
                                                    e.stopPropagation();
                                    ...
                                    {stats.loadingPending !== undefined &&
                                        stats.loadingPending > 0 && (
                                            <button
                                                type="button"
                                                onClick={(e) => {
                                                    e.stopPropagation();
            ...
            >
                <CardContent className={`${isCompact ? 'p-2.5' : 'p-6'} relative z-10`}>
                    <div className={`flex justify-between items-start ${isCompact ? 'mb-1' : 'mb-4'}`}>
                        ...
                        <div
                            className={`${isCompact ? 'text-lg' : 'text-2xl'} font-bold ${activeSection === 'database' ? 'text-white' : 'text-slate-800'}`}
                        >
                            •
                        </div>
                    </div>
                    <div>
                        <div
                            className={`${isCompact ? 'text-xs' : 'font-semibold'} mb-0.5 ${activeSection === 'database' ? 'text-indigo-50' : 'text-slate-600'}`}
                        >
                            {t('master_records', settings.language)}
                        </div>
                        <div
                            className={`text-[10px] sm:text-xs font-medium ${activeSection === 'database' ? 'text-indigo-100' : 'text-slate-400'}`}
                        >
