                ...
                <div className="max-h-[600px] overflow-y-auto">
                    <table className="w-full text-left text-sm">
                        <thead className="bg-slate-50 dark:bg-slate-950/50 sticky top-0 z-10">
                            <tr>
                                <th className="px-6 py-3 font-semibold text-slate-500 dark:text-slate-400">{t('timestamp', settings.language)}</th>
                                <th className="px-6 py-3 font-semibold text-slate-500 dark:text-slate-400">{t('actor', settings.language)}</th>
                                <th className="px-6 py-3 font-semibold text-slate-500 dark:text-slate-400">{t('action', settings.language)}</th>
                                <th className="px-6 py-3 font-semibold text-slate-500 dark:text-slate-400">{t('sheet_ref', settings.language)}</th>
                                <th className="px-6 py-3 font-semibold text-slate-500 dark:text-slate-400">{t('details', settings.language)}</th>
                            </tr>
                        </thead>
                            ...
                            {logs.length === 0 ? (
                                ...
                                <tr key={i} className="hover:bg-slate-50 dark:hover:bg-white/5 transition-colors">
                                    ...
                                    <td className="px-6 py-3">
                                        ...
                                        `}>
                                            {t(log.action.toLowerCase() as any, settings.language).replace(/_/g, ' ')}
                                        </span>
                                    </td>
