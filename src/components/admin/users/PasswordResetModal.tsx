...
export const PasswordResetModal: React.FC<PasswordResetModalProps> = ({ user, onClose, onSuccess, settings }) => {
    ...
    const handlePasswordReset = async (e: React.FormEvent) => {
        ...
        try {
            ...
            addToast('success', `${t('password', settings.language)} ${t('user_updated_successfully', settings.language)}: ${user.username} `);
            await onSuccess();
            onClose();
            setNewPassword('');
        } catch (error: any) {
    ...
    return (
        <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4">
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 w-full max-w-sm rounded-xl shadow-2xl p-6">
                ...
                <p className="text-sm text-slate-500 mb-4">{t('enter_new_password_for', settings.language)} <strong>{user.username}</strong></p>
                <form onSubmit={handlePasswordReset}>
                    ...
                    <div className="flex justify-end gap-2">
                        <Button type="button" variant="ghost" onClick={onClose}>{t('cancel', settings.language)}</Button>
