...
export function AnalyticsCharts({ sheets: allSheets, className, fullHeight = false }: AnalyticsChartsProps) {
    ...
    return (
        <div className={`${isCompact ? 'space-y-2' : 'space-y-4'} ${fullHeight ? 'h-full flex flex-col' : ''}`}>
            <div className="flex justify-end items-center gap-2 shrink-0">
                ...
                <div className="flex bg-slate-100 dark:bg-slate-800 p-1 rounded-lg border border-slate-200 dark:border-white/5">
                    {(['7D', '30D', '90D', 'CUSTOM'] as const).map((range) => (
                        ...
                        >
                            {range === '7D' ? '7 Days' : range === '30D' ? '30 Days' : range === '90D' ? '3 Months' : 'Custom'}
                        </button>
                    ))}
                </div>
            </div>
            ...
            {/* CUSTOM Date Picker */}
            {timeRange === 'CUSTOM' && (
                <div className="flex justify-end items-center gap-2 animate-in slide-in-from-top-2 duration-200 shrink-0">
            ...
            <div className={`grid grid-cols-1 md:grid-cols-2 ${isCompact ? 'gap-3' : 'gap-6'} animate-in fade-in slide-in-from-bottom-4 ${className} ${fullHeight ? 'flex-1 min-h-0' : ''}`}>
                {/* BAR CHART: Daily Activity */}
                <Card className={`shadow-lg shadow-slate-200/50 border-slate-200 dark:border-white/5 dark:bg-slate-900/50 bg-white/50 backdrop-blur-sm ${fullHeight ? 'flex flex-col h-full' : ''}`}>
                    <CardHeader className="shrink-0">
                        <CardTitle>Daily Activity</CardTitle>
                        <CardDescription>
                            Sheets created in the last {timeRange === '7D' ? '7 days' : timeRange === '30D' ? '30 days' : '3 months'}
                        </CardDescription>
                    </CardHeader>
                    <CardContent className={fullHeight ? 'flex-1 min-h-0' : (isCompact ? 'h-[200px]' : 'h-[300px]')}>
                ...
                <Card className={`shadow-lg shadow-slate-200/50 border-slate-200 dark:border-white/5 dark:bg-slate-900/50 bg-white/50 backdrop-blur-sm ${fullHeight ? 'flex flex-col h-full' : ''}`}>
                    ...
                    <CardContent className={fullHeight ? 'flex-1 min-h-0 flex flex-col' : (isCompact ? 'h-[200px]' : 'h-[300px]')}>
