...
export function OperationsMonitor({ sheets, onRefresh }: OperationsMonitorProps) {
    ...
    const supervisors = useMemo(() => {
        ...
        sheets.forEach(sheet => { if (sheet.supervisorName) s.add(sheet.supervisorName); });
        return Array.from(s).sort();
    }, [sheets]);

    const shifts = useMemo(() => {
        const s = new Set<string>();
        sheets.forEach(sheet => { if (sheet.shift) s.add(sheet.shift); });
        return Array.from(s).sort();
    }, [sheets]);
    ...
    const filteredSheets = useMemo(() => {
        return sheets.filter(sheet => {
            ...
            const matchesSearch = searchQuery === '' ||
                ...
                (sheet.vehicleNo && sheet.vehicleNo.toLowerCase().includes(searchQuery.toLowerCase())) ||
    ...
    const stats = useMemo(() => {
        ...
        return {
            total: active.length,
            staging: active.filter(s => s.status === SheetStatus.DRAFT || s.status === SheetStatus.STAGING_VERIFICATION_PENDING).length,
            audit: active.filter(s => s.status === SheetStatus.LOCKED).length,
            loading: active.filter(s => s.status === SheetStatus.LOADING_VERIFICATION_PENDING).length
        };
    }, [filteredSheets]);
    ...
    const ProgressStepper = ({ status }: { status: SheetStatus }) => {
        const stages = [
            { id: 'STAGING', label: t('staging', settings.language), active: [SheetStatus.DRAFT, SheetStatus.STAGING_VERIFICATION_PENDING, SheetStatus.LOCKED, SheetStatus.LOADING_VERIFICATION_PENDING, SheetStatus.COMPLETED].includes(status) },
            { id: 'AUDIT', label: 'Audit', active: [SheetStatus.LOCKED, SheetStatus.LOADING_VERIFICATION_PENDING, SheetStatus.COMPLETED].includes(status) },
            { id: 'LOADING', label: t('loading', settings.language), active: [SheetStatus.LOADING_VERIFICATION_PENDING, SheetStatus.COMPLETED].includes(status) },
            { id: 'DONE', label: t('completed', settings.language), active: status === SheetStatus.COMPLETED }
        ];
            ...
            <div className="flex items-center gap-1 w-full max-w-xs">
                {stages.map((stage, i) => (
                    <div key={stage.id} className="flex items-center flex-1 last:flex-none">
                        <div className={`h-1.5 flex-1 rounded-full transition-all duration-500 ${stage.active ? 'bg-primary' : 'bg-muted'}`} />
                        {i < stages.length - 1 && (
                            <ChevronRight size={10} className={stages[i + 1].active ? 'text-primary' : 'text-muted'} />
                        )}
                    </div>
                ))}
            </div>
        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
                <div className="lg:col-span-3 bg-card/40 backdrop-blur-xl border border-white/10 p-6 rounded-2xl shadow-xl flex flex-col md:flex-row gap-6 items-center justify-between">
                    <div className="space-y-1">
                        <h2 className="text-2xl font-black bg-gradient-to-br from-foreground to-foreground/50 bg-clip-text text-transparent flex items-center gap-2">
                            <TrendingUp className="text-primary" /> {t('operations_monitor', settings.language)}
                        </h2>
                        <p className="text-muted-foreground text-sm font-medium">{t('total_operational_active', settings.language)}: <span className="text-foreground">{stats.total}</span></p>
                    </div>

                    <div className="flex flex-wrap gap-3 justify-center md:justify-end">
                        <div className="px-4 py-2 rounded-xl bg-blue-500/10 border border-blue-500/20 text-center min-w-[100px]">
                            <p className="text-[10px] font-bold uppercase text-blue-500 opacity-70 mb-0.5">{t('staging_active', settings.language)}</p>
                        ...
                        <div className="px-4 py-2 rounded-xl bg-orange-500/10 border border-orange-500/20 text-center min-w-[100px]">
                            <p className="text-[10px] font-bold uppercase text-orange-500 opacity-70 mb-0.5">{t('audit_pending', settings.language)}</p>
                        ...
                        <div className="px-4 py-2 rounded-xl bg-purple-500/10 border border-purple-500/20 text-center min-w-[100px]">
                            <p className="text-[10px] font-bold uppercase text-purple-500 opacity-70 mb-0.5">{t('loading_active', settings.language)}</p>
                ...
                >
                    ...
                    <span className="font-bold text-xs uppercase tracking-widest">{t('refresh', settings.language)}</span>
                </Button>
            </div>
            ...
            <div className="grid grid-cols-1 gap-4 overflow-y-auto max-h-[calc(100vh-400px)] pr-2 custom-scrollbar">
                <AnimatePresence mode="popLayout">
                    {filteredSheets.length === 0 ? (
                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col items-center justify-center p-20 text-muted-foreground">
                            <AlertCircle size={48} className="mb-4 opacity-20" />
                            <p className="font-bold text-lg">{t('no_sheets_found', settings.language)}</p>
                        ...
                        filteredSheets.map((sheet) => (
                            ...
                            >
                                <Card
                                    ...
                                    onClick={() => {
                                        const isStaging = [SheetStatus.DRAFT, SheetStatus.STAGING_VERIFICATION_PENDING].includes(sheet.status);
                                ...
                                >
                                    <CardContent className="p-0">
                                        <div className="flex flex-col lg:flex-row items-stretch min-h-[100px]">
                                            {/* Status Pillar */}
                                            <div className={`w-2 ${sheet.status === SheetStatus.COMPLETED ? 'bg-emerald-500' :
                                                sheet.status === SheetStatus.LOCKED ? 'bg-orange-500' :
                                                    sheet.status === SheetStatus.LOADING_VERIFICATION_PENDING ? 'bg-purple-500' :
                                                        'bg-blue-500'
                                                }`} />
                                            ...
                                            <div className="flex-1 p-5 flex flex-col md:flex-row items-center gap-6">
                                                <div className="space-y-1 min-w-[140px]">
                                                    <div className="flex items-center gap-2">
                                                        <span className="text-[10px] font-black tracking-tighter text-muted-foreground uppercase">ID</span>
                                                        <span className="text-sm font-bold font-mono text-primary">#{sheet.id.slice(0, 8)}</span>
                                                    </div>
                                                    <div className="flex items-center gap-1.5">
                                                        <Clock size={12} className="text-muted-foreground" />
                                                        <span className="text-[10px] font-medium text-muted-foreground">
                                                            {new Date(sheet.updatedAt || sheet.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                                                        </span>
                                                    </div>
                                                </div>
                                                ...
                                                <div className="flex-1 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 w-full">
                                                    <div className="flex items-center gap-3">
                                                        ...
                                                        <div className="min-w-0">
                                                            <p className="text-[10px] font-bold text-muted-foreground uppercase leading-none mb-1">{t('destination', settings.language)}</p>
                                                            <p className="font-bold truncate group-hover:text-primary transition-colors">{sheet.destination || '---'}</p>
                                                        </div>
                                                    </div>
                                                    ...
                                                    <div className="flex items-center gap-3">
                                                        ...
                                                        <div className="min-w-0">
                                                            <p className="text-[10px] font-bold text-muted-foreground uppercase leading-none mb-1">{t('vehicle_no', settings.language)}</p>
                                                            <p className="font-bold truncate">{sheet.vehicleNo || '---'}</p>
                                                        </div>
                                                    </div>
                                                    ...
                                                    <div className="flex items-center gap-3">
                                                        ...
                                                        <div className="min-w-0">
                                                            <p className="text-[10px] font-bold text-muted-foreground uppercase leading-none mb-1">{t('supervisor', settings.language)}</p>
                                                            <p className="font-bold truncate">{sheet.supervisorName || '---'}</p>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                            ...
                                            <div className="flex flex-col md:flex-row items-center gap-6 p-5 bg-black/5 border-l border-white/5 min-w-[320px]">
                                                <div className="space-y-2 w-full">
                                                    <div className="flex items-center justify-between">
                                                        <span className="text-[10px] font-bold uppercase text-muted-foreground tracking-widest">{t('current_progress', settings.language)}</span>
                                                        <Badge variant="outline" className={`text-[10px] font-bold uppercase ${sheet.status === SheetStatus.COMPLETED ? 'border-emerald-500/30 text-emerald-500' :
                                                            sheet.status === SheetStatus.LOCKED ? 'border-orange-500/30 text-orange-500' :
                                                ...
                                                <Button size="sm" variant="outline" className="shrink-0 h-10 w-10 rounded-xl p-0 border-white/10 hover:bg-primary hover:text-white transition-all">
                                                    <ArrowRight size={18} />
                                                </Button>
                                            </div>
                                        </div>
