...
export function SecurityScanner() {
    ...
    const performLiveScan = async (): Promise<Vulnerability[]> => {
        ...
        if (!hasCSP) {
            findings.push({
                id: "WEB-CSP-001",
                severity: "HIGH",
                description: "Content Security Policy (CSP) not found in meta tags. This increases risk of XSS attacks.",
                status: "OPEN",
        ...
        const foundSensitive = Object.keys(localStorage).filter(k => sensitiveKeys.some(s => k.toLowerCase().includes(s)));
        if (foundSensitive.length > 0) {
            findings.push({
                ...
                status: "OPEN",
                remediation: "Ensure sensitive tokens are stored in HttpOnly cookies if possible, or encrypted."
            });
        }
        ...
        if ((window as any).webpackHotUpdate || (window as any).VITE_DEV_SERVER) {
            findings.push({
                id: "ENV-DEV-003",
                severity: "MEDIUM",
                description: "Dev Server / Hot Reloading artifacts detected. This should be disabled in production.",
                status: "OPEN",
        ...
        if (window.location.protocol !== 'https:' && window.location.hostname !== 'localhost') {
            findings.push({
                id: "NET-SSL-005",
                severity: "CRITICAL",
                description: "Insecure connection detected. Data is being transmitted over plain HTTP.",
                status: "OPEN",
        ...
        const dependencyVulnerabilities: Vulnerability[] = [
            {
                ...
                severity: "MEDIUM",
                package: "xlsx",
                description: "Old version of 'xlsx' detected. Vulnerable to potential resource exhaustion.",
                status: "RESOLVED",
            ...
            },
            {
                ...
                description: "Standard dialog package verified for accessibility and focus trap security.",
                status: "RESOLVED",
    ...
    const getSecurityGrade = () => {
        ...
        if (openIssues.length === 0) return { grade: "A+", label: "Safe", color: "text-emerald-500", border: "border-emerald-100", bg: "bg-emerald-50", icon: <ShieldCheck size={48} className="text-emerald-500" /> };

        const hasHigh = openIssues.some(r => r.severity === 'HIGH' || r.severity === 'CRITICAL');
        const hasMed = openIssues.some(r => r.severity === 'MEDIUM');

        if (hasHigh) return { grade: "D", label: "Critical Risks", color: "text-red-600", border: "border-red-100", bg: "bg-red-50", icon: <ShieldAlert size={48} className="text-red-500" /> };
        if (hasMed) return { grade: "C+", label: "Attention Required", color: "text-orange-600", border: "border-orange-100", bg: "bg-orange-50", icon: <ShieldAlert size={48} className="text-orange-500" /> };
        return { grade: "B", label: "Minor Issues", color: "text-blue-600", border: "border-blue-100", bg: "bg-blue-50", icon: <Shield size={48} className="text-blue-500" /> };
    };
        ...
        <div className="space-y-6 max-w-5xl mx-auto animate-in fade-in slide-in-from-bottom-4">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    ...
                    <p className="text-slate-500">Monitor system health and scan for vulnerabilities.</p>
                </div>
                <Button
                    onClick={startScan}
                    disabled={scanning}
                ...
                >
                    {scanning ? <RefreshCw className="mr-2 h-4 w-4 animate-spin" /> : <Shield className="mr-2 h-4 w-4" />}
                ...
                <div className="grid gap-6 md:grid-cols-3">
                    ...
                    <Card className="md:col-span-1 h-fit">
                        ...
                        <CardContent className="flex flex-col items-center">
                            <div className={`relative flex items-center justify-center w-32 h-32 rounded-full border-8 ${securityGrade.border} ${securityGrade.bg} mb-4`}>
                                {securityGrade.icon}
                            </div>
                            <div className={`text-3xl font-black ${securityGrade.color}`}>{securityGrade.grade}</div>
                            ...
                            <div className="w-full mt-6 space-y-3">
                                <div className="flex justify-between text-sm">
                                    ...
                                    <span className="font-bold text-slate-800">{filteredResults.filter(r => r.status === 'OPEN').length}</span>
                                </div>
                                <div className="flex justify-between text-sm">
                                    <span className="text-slate-500">Resolved Hidden</span>
                                    <span className="font-bold text-emerald-600">{dismissedIds.length}</span>
                                </div>
                                <div className="flex justify-between text-sm">
                                    <span className="text-slate-500">Last Scan</span>
                                    <span className="font-bold text-slate-800">{lastScan?.toLocaleTimeString()}</span>
                                </div>
                            </div>
                        </CardContent>
                    </Card>
                    ...
                    <Card className="md:col-span-2">
                        <CardHeader>
                            ...
                            <CardDescription>Identified security risks and recommendations</CardDescription>
                        </CardHeader>
                        <CardContent>
                                ...
                                <div className="space-y-4">
                                    {filteredResults.length === 0 ? (
                                        <div className="h-[300px] flex flex-col items-center justify-center text-center space-y-3">
                                            ...
                                            <div>
                                                <p className="font-bold text-slate-800">No Security Risks</p>
                                                <p className="text-sm text-slate-500">Your system is clean or all issues have been cleared.</p>
                                            </div>
                                        </div>
                                    ) : (
                                        filteredResults.map((vuln) => (
                                            <div key={vuln.id} className="p-4 rounded-lg border border-slate-100 bg-slate-50 hover:bg-white hover:shadow-md transition-all group">
                                                <div className="flex justify-between items-start mb-2">
                                                    <div className="flex items-center gap-2">
                                                        <Badge className={getSeverityColor(vuln.severity)} variant="outline">
                                                            {vuln.severity}
                                                        </Badge>
                                                        <span className="font-mono text-xs text-slate-400">{vuln.id}</span>
                                                    </div>
                                                    <div className="flex items-center gap-2">
                                                        <Badge variant={vuln.status === "RESOLVED" ? "default" : "outline"} className={vuln.status === "RESOLVED" ? "bg-emerald-500" : "text-slate-500"}>
                                                            {vuln.status}
                                                        </Badge>
                                                        {vuln.status === "RESOLVED" && (
                                                            <button
                                                                onClick={(e) => { e.stopPropagation(); handleDismiss(vuln.id); }}
                                                ...
                                                <h4 className="font-bold text-slate-800 text-sm mb-1 flex items-center gap-2">
                                                    {vuln.package && <Server size={14} className="text-slate-400" />}
                                                    {vuln.package ? `Package: ${vuln.package}` : "System Configuration"}
                                                </h4>
            ...
            {!scanning && !results && (
                <div className="text-center py-12 bg-slate-50 rounded-xl border border-dashed border-slate-200">
                    ...
                    <p className="text-slate-500 max-w-md mx-auto mt-2 mb-6">Run a diagnostic scan to check for dependency vulnerabilities, configuration issues, and code integrity risks.</p>
                    <Button variant="outline" onClick={startScan}>Start Initial Scan</Button>
                </div>
            )}
        </div>
    );
