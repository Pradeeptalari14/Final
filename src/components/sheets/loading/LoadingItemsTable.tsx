                ...
                <table className="w-full text-xs border-collapse">
                    <thead className="bg-slate-50 text-slate-600 font-bold border-b border-slate-200">
                        <tr>
                            <th
                                rowSpan={2}
                                className="p-3 text-center w-12 sticky left-0 bg-slate-50 z-20 border-r border-slate-200 border-b"
                            >
                                Sr.No
                            </th>
                            {canReject && (
                                <th
                                    rowSpan={2}
                                    className="p-3 text-center w-8 sticky left-12 bg-slate-50 z-20 border-r border-slate-200 border-b"
                                >
                                    Act
                                </th>
                            )}
                            <th
                                rowSpan={2}
                                className={`p-3 text-left min-w-[200px] sticky bg-slate-50 z-20 border-r border-slate-200 border-b ${canReject ? 'left-20' : 'left-12'}`}
                            >
                                SKU Name
                            </th>
                            ...
                            <th
                                colSpan={4}
                                className="p-2 text-center border-r border-b border-slate-200 text-slate-500 bg-slate-50 text-[10px] uppercase tracking-wider"
                            >
                                STAGING QTY
                            </th>
                            <th
                                colSpan={13}
                                className="p-2 text-center border-b border-slate-200 border-l-4 border-l-slate-800 text-slate-800 bg-slate-50 text-[10px] uppercase tracking-wider"
                            >
                                LOADING QTY
                            </th>
                        </tr>
                        <tr>
                            ...
                            <th className="p-2 text-center w-20 border-r border-slate-200 text-slate-500 text-[10px] uppercase leading-tight">
                                Cases/
                                <br />
                                Pallet
                            </th>
                            ...
                            <th className="p-2 text-center w-16 border-r border-slate-200 bg-blue-50/50 text-blue-700 text-[10px] font-bold uppercase">
                                TOTAL
                            </th>
                    ...
                    <tbody>
                        {currentSheet.loadingItems
                            ?.filter((li) => !showRejectedOnly || li.isRejected)
                            .map((lItem, idx) => {
                                ...
                                return Array.from({ length: rowsNeeded }).map((_, rIndex) => (
                                    <tr
                                        ...
                                        className={cn(
                                            `hover:bg-blue-50/20 ${lItem.isRejected ? 'bg-red-50/50' : idx % 2 === 0 ? 'bg-white' : 'bg-slate-50/30'}`
                                        )}
                                    >
                                        ...
                                        {rIndex === 0 && (
                                            <>
                                                <td
                                                    rowSpan={rowsNeeded}
                                                    className={cn(
                                                        'border-r border-b border-slate-200 p-3 text-center font-bold font-mono align-middle sticky left-0 z-10 w-12 text-slate-900 bg-inherit',
                                                        lItem.isRejected
                                                ...
                                                {canReject && (
                                                    <td
                                                        rowSpan={rowsNeeded}
                                                        className="border-r border-b border-slate-200 p-0 text-center align-middle sticky left-12 z-10 bg-inherit w-8"
                                                    >
                                                        <Button
                                                            variant="ghost"
                                                            size="icon"
                                                            ...
                                                            className={cn(
                                                                ...
                                                                lItem.isRejected
                                                                    ? 'text-red-600 bg-red-100 hover:bg-red-200 scale-110'
                                                ...
                                                <td
                                                    rowSpan={rowsNeeded}
                                                    className={cn(
                                                        `border-r border-b border-slate-200 p-3 font-bold align-middle sticky z-10 bg-inherit min-w-[200px] text-slate-900 ${canReject ? 'left-20' : 'left-12'}`,
                                                        lItem.isRejected ? 'text-red-700' : ''
                                                    )}
                                                >
                                                    {sItem.skuName}
                                                </td>
                                                ...
                                                <td
                                                    rowSpan={rowsNeeded}
                                                    className="border-r border-b border-slate-200 p-3 text-center text-slate-500"
                                                >
                                                    {sItem.casesPerPlt}
                                                </td>
                                                <td
                                                ...
                                                <td
                                                    rowSpan={rowsNeeded}
                                                    className="border-r border-b border-slate-200 p-3 text-center font-bold bg-blue-50/30 text-blue-700"
                                                >
                                                    {sItem.ttlCases}
                                                </td>
                                            </>
                                        ...
                                        {Array.from({ length: 10 }).map((_, cIndex) => {
                                            ...
                                            return (
                                                <td
                                                    key={cIndex}
                                                    className={`border-r border-b border-slate-100 p-0 text-center w-10 h-10 ${!isValid ? 'bg-slate-50' : ''} ${cIndex === 0 ? 'border-l-4 border-l-slate-800' : ''}`}
                                                >
                                                    {isValid && (
                                                        <input
                                        ...
                                        {rIndex === 0 && (
                                            <>
                                                ...
                                                <td
                                                    rowSpan={rowsNeeded}
                                                    className="border-r border-b border-slate-100 p-3 text-center font-bold text-slate-900"
                                                >
                                                    {lItem.total}
                                                </td>
                                                <td
            ...
            <div className="md:hidden divide-y divide-slate-100">
                {currentSheet.loadingItems?.map((lItem, idx) => {
                    ...
                    return Array.from({ length: rowsNeeded }).map((_, rIndex) => (
                        <div
                            key={`${lItem.skuSrNo}-${rIndex}`}
                            className={`p-4 ${idx % 2 === 0 ? 'bg-white' : 'bg-slate-50/50'}`}
                        >
                            <div className="flex justify-between items-start mb-3">
                                <div>
                                    <div className="text-[10px] font-bold text-slate-400 uppercase">
                                        ITEM #{lItem.skuSrNo}{' '}
                                        {rowsNeeded > 1 && `(Row ${rIndex + 1})`}
                                    </div>
                                ...
                                {rIndex === 0 && (
                                    <div className="text-right">
                                        ...
                                        <div
                                            className={`text-sm font-bold ${lItem.balance === 0 ? 'text-green-600' : 'text-amber-600'}`}
                                        >
                                            {lItem.balance}
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex-1">
                                        ...
                                        <input
                                            ...
                                            onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                                                onLooseChange(lItem.skuSrNo, e.target.value)
                                            }
                                            disabled={isLocked}
