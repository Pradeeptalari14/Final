...
export const AdditionalItemsSection: React.FC<AdditionalItemsSectionProps> = ({
    ...
    const isReadOnly = status === SheetStatus.COMPLETED || status === SheetStatus.LOADING_VERIFICATION_PENDING;
    const itemsToDisplay = isReadOnly
        ? (additionalItems || []).filter(item => (item.skuName && item.skuName.trim() !== '') || item.total > 0)
        : (additionalItems || []);
    ...
    return (
        <div className="p-4 border-b border-slate-200 bg-slate-50/30">
                    ...
                    <thead>
                        <tr className="bg-slate-50 text-slate-500">
                            ...
                            {Array.from({ length: 10 }).map((_, i) => (
                                <th key={i} className="p-2 text-center border-r w-8">{i + 1}</th>
                            ))}
                    ...
                    <tbody>
                        {itemsToDisplay.map((item) => (
                            <tr key={item.id} className="border-t border-slate-100">
                                <td className="p-2 text-center border-r text-slate-900 font-bold">{item.id}</td>
                                ...
                                {item.counts.map((c, idx) => (
                                    <td key={idx} className="p-0 border-r">
                                        <input
                                            ...
                                            value={c || ''}
                                            onChange={e => onItemChange(item.id, 'count', e.target.value, idx)}
                                            disabled={isLocked}
                                        />
                                    </td>
                                ))}
                                <td className="p-2 text-center font-bold text-slate-900">{item.total}</td>
                            </tr>
                        ))}
                    </tbody>
                    <tfoot>
            ...
            <div className="md:hidden space-y-4">
                {itemsToDisplay.map((item) => (
                    <div key={item.id} className="bg-white border border-slate-200 rounded-lg p-3 space-y-3">
                        <div className="flex justify-between items-center">
                            <span className="text-[10px] font-bold text-slate-400">EXTRA #{item.id}</span>
                            <span className="text-sm font-bold text-blue-600">Total: {item.total}</span>
                        </div>
                        <SkuSelector
                            value={item.skuName || ''}
                            onChange={val => onItemChange(item.id, 'skuName', val)}
