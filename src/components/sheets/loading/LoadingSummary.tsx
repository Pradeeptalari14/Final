            ...
            <div className="p-4 md:p-6 bg-amber-50/20">
                ...
                <div className="mb-3 space-y-1">
                    {returnedItems.map((item, idx) => (
                        <div
                            key={`ret-${idx}`}
                            className="text-xs text-amber-700 bg-amber-50 p-2 rounded border border-amber-100"
                        >
                            For{' '}
                            <strong>
                                {
                                    currentSheet.stagingItems.find((s) => s.srNo === item.skuSrNo)
                                        ?.skuName
                                }
                            </strong>{' '}
                    ...
                    {overLoadedItems.map((item, idx) => (
                        <div
                            key={`ol-${idx}`}
                            className="text-xs text-blue-700 bg-blue-50 p-2 rounded border border-blue-100"
                        >
                            For{' '}
                            <strong>
                                {
                            ...
                            {Math.abs(item.balance)} Cases Extra loaded.
                        </div>
                    ))}
                    {extraItemsWithQty.map((item, idx) => (
                        <div
                            ...
                            className="text-xs text-blue-700 bg-blue-50 p-2 rounded border border-blue-100"
                        >
                            For <strong>{item.skuName}</strong> {item.total} Cases Extra
                            (Additional) loaded.
                        </div>
                    ))}
                </div>
