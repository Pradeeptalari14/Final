...
export const StagingMobileCards: React.FC<StagingMobileCardsProps> = ({ items, status, onUpdateItem }) => {
                ...
                {displayItems.map((item, index) => (
                    <div key={index} className="p-4 bg-white space-y-3">
                        <div className="flex justify-between items-center mb-1">
                            <span className="bg-slate-100 text-slate-500 text-[10px] px-2 py-0.5 rounded font-mono font-bold">ITEM #{isReadOnly ? index + 1 : item.srNo}</span>
                        ...
                        <div className="grid grid-cols-3 gap-2">
                            <div className="space-y-1">
                                <label className="text-[10px] text-slate-400 uppercase font-bold px-1">Cs/Plt</label>
                                <input
                                    type="number"
                            ...
                            <div className="space-y-1">
                                <label className="text-[10px] text-slate-400 uppercase font-bold px-1">Full Plt</label>
                                <input
                                    type="number"
                                    value={item.fullPlt || ''}
                            ...
                            <div className="space-y-1">
                                <label className="text-[10px] text-slate-400 uppercase font-bold px-1 text-amber-600">Loose</label>
                                <input
                                    type="number"
                                    value={item.loose || ''}
