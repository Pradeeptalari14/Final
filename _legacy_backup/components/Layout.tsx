...
export const Layout: React.FC<LayoutProps> = ({ children, currentPage, onNavigate }) => {
    ...
    const NavItem = ({
        ...
        if (!currentUser || (!roles.includes(currentUser.role) && currentUser.role !== Role.ADMIN))
            return null;
            ...
            >
                ...
                <nav className="flex-1 py-2 space-y-1 overflow-y-auto">
                    ...
                    <NavItem
                        ...
                        roles={[Role.ADMIN]}
                    />
                    <NavItem page="audit" icon={History} label="Audit Logs" roles={[Role.ADMIN]} />
                </nav>
                                        ...
                                        {notifications.length === 0 ? (
                                            ...
                                            <div className="divide-y divide-slate-50">
                                                {notifications.map((n) => (
                                                    <div
                                                        key={n.id}
                                                        className={`p-3 hover:bg-slate-50 transition-colors ${!n.read ? 'bg-blue-50/50' : ''}`}
                                                    >
                    ...
                    <div className="bg-white rounded-xl shadow-2xl w-full max-w-sm overflow-hidden transform transition-all">
                        ...
                        <form onSubmit={handleChangePasswordSubmit} className="p-6 space-y-4">
                            ...
                            <div className="pt-2">
                                <button
                                    type="submit"
                                    className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-2.5 rounded-lg shadow-md transition-all active:scale-[0.98]"
                                >
                                    Update Password
                                </button>
                            </div>
