...
export const StagingSheet: React.FC<Props> = ({
    existingSheet,
    onCancel,
    onLock,
    ...
    const isLocked =
        ((existingSheet?.status === SheetStatus.LOCKED ||
            existingSheet?.status === SheetStatus.COMPLETED ||
            existingSheet?.status === SheetStatus.STAGING_VERIFICATION_PENDING) &&
            currentUser?.role !== Role.ADMIN) ||
        (existingSheet?.status === SheetStatus.DRAFT &&
            existingSheet.createdBy !== currentUser?.username &&
            currentUser?.role !== Role.ADMIN);
    ...
    const [supervisorName, setSupervisorName] = useState(
        existingSheet?.supervisorName || currentUser?.fullName || currentUser?.username || ''
    );
    ...
    const [items, setItems] = useState<any[]>(() => {
        ...
        const targetRows = 20;
        const rows =
            initial.length < targetRows
                ? [
                      ...initial,
                      ...Array.from({ length: targetRows - initial.length }, (_, i) => ({
                          srNo: initial.length + i + 1,
                          skuName: '',
                          casesPerPlt: '',
                          fullPlt: '',
                          loose: '',
    ...
    const handleSave = async (lock: boolean, e?: React.MouseEvent) => {
        ...
        try {
            ...
            const sheetId =
                existingSheet?.id ||
                `SH-${Date.now().toString(36).toUpperCase()}-${Math.random().toString(36).substring(2, 6).toUpperCase()}`;
            ...
            if (lock) {
                const validStagingItems = finalItems.filter(
                    (item) => item.skuName && item.ttlCases > 0
                );
                finalLoadingItems = validStagingItems.map((sItem) => {
                    const existingLoadingItem = finalLoadingItems.find(
                        (li) => li.skuSrNo === sItem.srNo
                    );
                    if (existingLoadingItem) {
                        return {
                            ...existingLoadingItem,
                            balance: sItem.ttlCases - existingLoadingItem.total
                        };
                    } else {
                        return {
                            skuSrNo: sItem.srNo,
                            ...
                            looseInput: 0,
                            total: 0,
                            balance: sItem.ttlCases
                        };
                    }
                });
                if (finalAdditionalItems.length === 0) {
                    finalAdditionalItems = Array.from({ length: 5 }, (_, i) => ({
                        id: i + 1,
                        skuName: '',
    ...
    const handleApprove = async (approve: boolean) => {
        ...
        if (approve) {
            if (confirm('Confirm approval of this Staging Sheet? It will be locked for loading.')) {
                ...
                if (
                    (!updatedSheet.loadingItems || updatedSheet.loadingItems.length === 0) &&
                    updatedSheet.stagingItems
                ) {
                    const validStagingItems = updatedSheet.stagingItems.filter(
                        (item) => item.skuName && item.ttlCases > 0
                    );
                    updatedSheet.loadingItems = validStagingItems.map((sItem) => ({
                        skuSrNo: sItem.srNo,
                        cells: [],
                        looseInput: 0,
                        total: 0,
                    ...
                    updatedSheet.additionalItems = Array.from({ length: 5 }, (_, i) => ({
                        id: i + 1,
                        skuName: '',
                        counts: Array(10).fill(0),
                        total: 0
    ...
    return (
        <div className="bg-white shadow-xl shadow-slate-200 rounded-xl pb-60 relative border border-slate-100 flex flex-col min-h-full">
            ...
            {!isPreview && (
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center p-4 md:p-6 border-b border-slate-100 bg-white print:hidden gap-4">
                    <div>
                        <button
                            onClick={handleBack}
                            className="text-slate-500 hover:text-blue-600 text-sm flex items-center gap-1 mb-2 transition-colors"
                        >
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            {isLocked && !isPendingApproval && (
                                <span className="bg-orange-100 text-orange-700 text-xs px-2.5 py-1 rounded-full font-bold border border-orange-200">
                                    LOCKED
                                </span>
                            )}
                            {isPendingApproval && (
                                <span className="bg-purple-100 text-purple-700 text-xs px-2.5 py-1 rounded-full font-bold border border-purple-200 flex items-center gap-1">
                    ...
                    <div className="flex gap-2">
                        <button
                            ...
                            className="bg-slate-800 text-white px-4 py-2 rounded-lg text-sm font-medium flex items-center gap-2 hover:bg-slate-700 transition-colors"
                        >
                    ...
                    <div className="mx-6 mt-4 p-4 bg-red-50 border border-red-200 rounded-xl flex items-start gap-3 animate-in fade-in slide-in-from-top-2">
                        ...
                        <div>
                            ...
                            <p className="text-red-500 text-xs mt-2">
                                Please address the feedback above and re-submit for approval.
                            </p>
                        </div>
                    </div>
                )}
            ...
            {isPreview && (
                <div className="bg-slate-800 text-white p-4 rounded-b-xl shadow-lg flex justify-between items-center print:hidden sticky top-0 z-50">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h3 className="font-bold">Excel Print View</h3>
                        </div>
                    </div>
            ...
            >
                <div className="min-w-[800px]">
                    <table className="w-full border-collapse border border-black mb-1">
                        ...
                        <tbody>
                            <tr>
                                <td className="border border-black p-1 font-bold text-center bg-gray-100 w-10">
                                    Shift
                                </td>
                                <td
                                    colSpan={3}
                                ...
                                <td className="border border-black p-1 font-bold w-24">
                                    {loadingDockNo}
                                </td>
                            </tr>
                            <tr>
                                <td
                                    rowSpan={2}
                                    className="border border-black p-1 text-center font-bold text-xl align-middle"
                                >
                                    {shift}
                                </td>
                    ...
                    <div className="border border-black">
                        <div className="w-full">
                            <div className="font-bold text-center bg-gray-200 border-b border-black p-1">
                                STAGING DETAILS
                            </div>
            ...
            <div
                className={`p-4 md:p-6 bg-slate-50 border-b border-slate-100 ${isPreview ? 'hidden' : 'block'} print:hidden`}
            >
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
                    <div>
                        ...
                        <select
                            ...
                            disabled={isLocked}
                            className="w-full border border-slate-200 bg-white p-2.5 rounded-lg text-sm text-slate-700 outline-none"
                        >
                    ...
                    <div>
                        ...
                        <input
                            ...
                            disabled={isLocked}
                            className="w-full border border-slate-200 bg-slate-100 p-2.5 rounded-lg text-sm text-slate-600 outline-none cursor-not-allowed"
                        />
                    </div>
                    <div>
                        <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1.5 flex items-center gap-1">
                        ...
                        <input
                            ...
                            disabled={isLocked}
                            className="w-full border border-slate-200 bg-white p-2.5 rounded-lg text-sm text-slate-700 outline-none placeholder:text-slate-300"
                            placeholder="Enter Destination"
                        />
                    </div>
                    <div>
                        <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1.5 flex items-center gap-1">
                            <User size={14} /> Supervisor Name *
                        </label>
                        <input
                            type="text"
                    ...
                    <div>
                        <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1.5 flex items-center gap-1">
                            <Truck size={14} /> Loading Dock No
                        </label>
                        <input
                            type="text"
                            value={loadingDockNo}
                            onChange={(e) => {
                                setLoadingDockNo(e.target.value);
                                setIsDirty(true);
                            }}
                            disabled={isLocked}
                            className="w-full border border-slate-200 bg-white p-2.5 rounded-lg text-sm text-slate-700 outline-none placeholder:text-slate-300"
            ...
            <div
                className={`p-4 md:p-6 ${isPreview ? 'hidden' : 'block'} print:hidden flex-1 flex flex-col`}
            >
                <div className="overflow-x-auto rounded-xl border border-slate-200 shadow-sm flex-1 custom-scrollbar">
                    ...
                    <table className="w-full min-w-[600px] text-sm">
                        <thead className="bg-slate-50 text-slate-600 border-b border-slate-200">
                            <tr>
                                <th className="p-3 w-12 text-center font-semibold text-xs uppercase tracking-wider text-slate-400">
                                    #
                                </th>
                                <th className="p-3 text-left font-semibold text-xs uppercase tracking-wider min-w-[150px]">
                                    SKU Name
                                </th>
                                <th className="p-3 w-24 text-center font-semibold text-xs uppercase tracking-wider">
                                    Cases/PLT
                                </th>
                                <th className="p-3 w-24 text-center font-semibold text-xs uppercase tracking-wider">
                                    Full PLT
                                ...
                                <th className="p-3 w-24 text-center font-semibold text-xs uppercase tracking-wider bg-blue-100 text-blue-900 border-l border-blue-200">
                                    Total
                                </th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-100">
                            {items.map((item, index) => (
                                <tr
                                    key={item.srNo}
                                    className={`hover:bg-blue-50/30 transition-colors group ${index % 2 === 0 ? 'bg-white' : 'bg-slate-50/30'}`}
                                >
                                    ...
                                    <td className="p-1">
                                        <input
                                            ...
                                            value={item.skuName}
                                            onChange={(e) =>
                                                handleItemChange(index, 'skuName', e.target.value)
                                            }
                                            disabled={isLocked}
                                            className="w-full p-2 bg-transparent rounded hover:bg-white focus:bg-white focus:ring-2 focus:ring-blue-500/20 outline-none transition-all placeholder:text-slate-300"
                                    ...
                                    <td className="p-1">
                                        <input
                                            ...
                                            disabled={isLocked}
                                            className="w-full p-2 text-center bg-transparent rounded hover:bg-white focus:bg-white focus:ring-2 focus:ring-blue-500/20 outline-none transition-all"
                                            placeholder="-"
                                        />
                                    </td>
                                    <td className="p-1">
                                        <input
                                            ...
                                            value={item.fullPlt}
                                            onChange={(e) =>
                                                handleItemChange(index, 'fullPlt', e.target.value)
                                            }
                                            disabled={isLocked}
                                            className="w-full p-2 text-center bg-transparent rounded hover:bg-white focus:bg-white focus:ring-2 focus:ring-blue-500/20 outline-none transition-all"
                                            placeholder="-"
                                    ...
                                    <td className="p-1">
                                        <input
                                            ...
                                            value={item.loose}
                                            onChange={(e) =>
                                                handleItemChange(index, 'loose', e.target.value)
                                            }
                                            disabled={isLocked}
                                            className="w-full p-2 text-center bg-transparent rounded hover:bg-white focus:bg-white focus:ring-2 focus:ring-blue-500/20 outline-none transition-all"
                                            placeholder="-"
                                    ...
                                    <td className="p-3 text-center font-bold text-blue-700 bg-blue-50 border-l border-blue-100">
                                        {item.ttlCases || '-'}
                                    </td>
                                </tr>
                            ))}
                        ...
                        <tfoot className="bg-slate-50 font-bold border-t border-slate-200">
                            <tr>
                                ...
                                <td className="p-4 text-center text-blue-600 text-lg bg-white shadow-inner">
                                    {totalQty}
                                </td>
                            </tr>
                        </tfoot>
                ...
                {!isLocked && (
                    <div className="mt-4 flex justify-center pb-4">
                        <button
                            ...
                            className="w-full flex items-center justify-center gap-2 text-sm font-bold text-blue-600 hover:text-blue-700 bg-blue-50 hover:bg-blue-100 py-3 rounded-lg transition-colors border-2 border-dashed border-blue-200 hover:border-blue-300"
                        >
            ...
            {!isLocked && !isPreview && (
                <div className="fixed bottom-0 left-0 right-0 p-4 bg-white/90 backdrop-blur-md border-t border-slate-200 shadow-[0_-4px_20px_-5px_rgba(0,0,0,0.1)] flex justify-center gap-4 z-40 lg:ml-64 print:hidden">
                    <button
                        ...
                        className="px-6 py-2.5 bg-white text-slate-600 border border-slate-300 rounded-lg hover:bg-slate-50 flex items-center gap-2 shadow-sm font-medium transition-all text-sm"
                    >
                    ...
                    <button
                        ...
                        className="px-8 py-2.5 bg-purple-600 text-white rounded-lg hover:bg-purple-700 flex items-center gap-2 font-bold shadow-lg shadow-purple-500/30 transform hover:scale-[1.02] active:scale-[0.98] transition-all text-sm"
                    >
                        <Lock size={18} /> Request Verification
                    </button>
                </div>
            )}
                    ...
                    <div className="w-full max-w-2xl bg-white border border-purple-200 rounded-xl p-4 shadow-sm">
                        ...
                        <div className="grid sm:grid-cols-3 gap-3">
                            <label className="flex items-center gap-2 text-sm text-slate-700 cursor-pointer p-2 hover:bg-purple-50 rounded-lg transition-colors border border-transparent hover:border-purple-100">
                                <div
                                    className={`w-5 h-5 rounded border flex items-center justify-center transition-colors ${stagingChecks.qty ? 'bg-purple-600 border-purple-600 text-white' : 'bg-white border-slate-300'}`}
                                >
                                ...
                                <input
                                    type="checkbox"
                                    className="hidden"
                                    checked={stagingChecks.qty}
                                    onChange={() =>
                                        setStagingChecks((prev) => ({ ...prev, qty: !prev.qty }))
                                    }
                                />
                            ...
                            <label className="flex items-center gap-2 text-sm text-slate-700 cursor-pointer p-2 hover:bg-purple-50 rounded-lg transition-colors border border-transparent hover:border-purple-100">
                                <div
                                    className={`w-5 h-5 rounded border flex items-center justify-center transition-colors ${stagingChecks.condition ? 'bg-purple-600 border-purple-600 text-white' : 'bg-white border-slate-300'}`}
                                >
                                    {stagingChecks.condition && (
                                ...
                                <input
                                    type="checkbox"
                                    className="hidden"
                                    checked={stagingChecks.condition}
                                    onChange={() =>
                                        setStagingChecks((prev) => ({
                                            ...prev,
                            ...
                            <label className="flex items-center gap-2 text-sm text-slate-700 cursor-pointer p-2 hover:bg-purple-50 rounded-lg transition-colors border border-transparent hover:border-purple-100">
                                <div
                                    className={`w-5 h-5 rounded border flex items-center justify-center transition-colors ${stagingChecks.sign ? 'bg-purple-600 border-purple-600 text-white' : 'bg-white border-slate-300'}`}
                                >
                                    {stagingChecks.sign && (
                                        <CheckCircle size={12} strokeWidth={4} />
                                    )}
                                ...
                                <input
                                    type="checkbox"
                                    className="hidden"
                                    checked={stagingChecks.sign}
                                    onChange={() =>
                                        setStagingChecks((prev) => ({ ...prev, sign: !prev.sign }))
                                    }
                                />
