...
// Lazy load widgets to prevent circular dependency / initialization issues
const StaffPerformanceWidget = React.lazy(() =>
    import('./StaffPerformanceWidget').then((module) => ({
        default: module.StaffPerformanceWidget
    }))
);
const SLAMonitorWidget = React.lazy(() =>
    import('./SLAMonitorWidget').then((module) => ({ default: module.SLAMonitorWidget }))
);
